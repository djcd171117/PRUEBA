...
# ==============================================================================
# CAPA 2: EL CEREBRO (IA + GOOGLE + TEMPORALIDAD)
# ==============================================================================
...
def obtener_contexto_detallado_google(lat, lon):
    try:
        ...
        return {'es_mall': es_m, 'nse_google': nse_g, 'patron_flujo': patron}
    except:
        return {'es_mall': False, 'nse_google': None, 'patron_flujo': "No detectado"}

def clasificar_micro_entorno(p_geom, edif, denue, es_m_g):
    if es_m_g: return "Plaza Comercial / Retail Hub"
    ...
    if area > 1500: return "Lifestyle Center / Zona Alto Valor" if locales > 3 else "Tienda Ancla / Big Box"
...
def evaluar_local_comercial(lat, lon, giro_scian):
    ...
    tipo = clasificar_micro_entorno(p_geom, edif, df_h, ctx_g['es_mall'])
    ...
    X = pd.DataFrame([{'codigo_act': str(giro_scian), 'dist_competidor_m': dist_c, 'm2_construccion_50m': masa, 'dist_ancla_urbana_m': 150.0, 'dist_esquina_m': 20.0, 'tipologia_urbana': tribu, 'centralidad_flujo': cent_v, 'segmento_nse': nse}])
    ...
    ctx = {'tipo_predio': tipo, 'segmento_nse': nse, 'patron_flujo': ctx_g['patron_flujo'], 'dias_pico': d_pico, 'masa_critica': masa, 'potencial_renta': "Alto" if v_temp > 1.3 else "Moderado", 'conectividad': "Alta" if cent_v > 0.006 else "Local"}
    return [1-p_ex, p_ex], ctx, X.iloc[0]

# ==============================================================================
# CAPA 3 & 4: INICIALIZACIÓN E INTERFAZ
# ==============================================================================
...
with c_map:
    ...
    folium.Marker([st.session_state.c_lat, st.session_state.c_lng], icon=folium.Icon(color='purple', icon='star')).add_to(m)
    map_dict = st_folium(m, width="100%", height=550, key=f"map_{st.session_state.c_lat}")
...
if st.session_state.analisis:
    ...
    with t2:
        ...
        st.info("Escolaridad: Superior" if info['segmento_nse'] == "Premium" else "Escolaridad: Media")
    with t3:
        st.metric("Días Pico", info.get('dias_pico', "N/A"))
        st.metric("Potencial Renta", info.get('potencial_renta', "Moderado"))
    ...
    with t4:
        ...
        st.download_button("📥 Descargar Reporte", data=csv, file_name=f"Dictamen_{st.session_state.c_lat:.4f}.csv", mime="text/csv")
